...
class Document(models.Model):
    ...
    state = fields.Selection([('open', 'Aberto'), ('lock', 'Bloqueado')], string='State', default='open', tracking=True, copy=False)
    ...
    def compute_admin_group(self):
        if self.env.user.has_group('gz_finance_docs.group_document_hub_document_administrator'):
            self.is_admin = True
        else:
            self.is_admin = False
